...
요청받은 비즈니스 요구사항이나 리팩토링의 직접적인 범위와 무관한 주변 코드(작동에 영향을 주지 않는 불필요한 스타일 수정, 사소한 레이아웃 변경 등)는 임의로 수정하지 않고, 최소 변경 범위만을 정밀하게 조준하여 반영합니다. 단일 사용 코드를 위한 불필요한 추상화나 요청되지 않은 설정성(Configurability)을 일절 추가하지 않으며, 더 가볍게 작성할 수 있다면 최소화하여 재작성합니다.
...
### 테스트 및 검증 규격 및 하이브리드 아키텍처
- 모든 테스트는 **pytest** 프레임워크를 기반으로 실행하고 검증합니다.
- 외부에 노출되는 모든 Public API 및 코어 로직은 **100% 유닛 테스트**를 작성하여 동작을 입증해야 합니다.
- **Prompt Server (`apps/server`) 하이브리드 테스트 아키텍처**:
  - 데이터베이스 접근, ORM 연산, DRF View 및 Serializer를 교차 검증하는 서버 모델 테스트는 반드시 `django.test.TestCase`를 상속받고 `setUpTestData(cls)`를 활용하여 테스트 오버헤드를 극소화합니다.
  - 반면 데이터베이스 접근이 완전히 부재하고 순수한 텍스트 파싱, 컴파일 연산 및 헬퍼 기능을 검증하는 독립 유틸리티성 테스트는 `unittest.TestCase`를 상속받도록 격리 설계하여 초고속 개발자 피드백 루프를 수호합니다.
