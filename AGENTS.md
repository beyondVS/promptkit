...
1. **상위 플랫폼·시스템 지침 및 법적·보안 제약**: 플랫폼 또는 실행 환경이 직접 제공하는 최상위 규격과 법적·보안 요구사항
2. **사용자의 명시적 요청**: 대화에서 전달된 직전 지시사항. 단, 1번과 충돌해서는 안 됨
3. **프로젝트 환경 설정 및 헌법**: (예: `package.json`, `tsconfig.json`, `.eslintrc`, `.prettierrc`, [constitution.md](.specify/memory/constitution.md))에 명시된 기계적·설계 규칙
4. **프로젝트별 지침 및 `AGENTS.md` / 하위 모듈 문서**: 아키텍처 가이드와 수정 대상 모듈의 지침 및 공개 기본 규칙
...
### 3.4 컨텍스트 수집 및 비신뢰 데이터 경계 (Context & Trust Boundaries)
...
### 3.5 보안 및 데이터 무결성 (Security & Integrity)
...
4. **조건부 자가 치유 루프 (Conditional Self-healing)**
   ...
   - **중단 경계**: 같은 가설이 검증으로 반증되거나, 새 증거·안전한 다음 조치 없이 재시도를 반복하게 되면 중단하고 증거와 다음 선택지를 보고하십시오. 횟수 자체를 종료 기준으로 고정하지 않습니다.

5. **구현 완료 후 교차 검증 (Post-Implementation Cross-Validation)**
   - `speckit-implement`가 실제 구현 변경을 완료한 경우에는 `auditor`를 **반드시** 호출합니다. 변경이 없으면 그 이유와 미검증 범위를 기록하며 감사 호출은 생략할 수 있습니다.
   - 그 외에는 사용자가 명시적으로 감사·검수를 지시했거나, 보안·데이터 마이그레이션·운영 변경·대규모 설계처럼 실패 비용이 큰 작업일 때만 `auditor`를 선택적으로 호출합니다.
...
`speckit-implement`가 실제 구현 변경을 완료한 경우, 메인 에이전트는 `auditor`를 **반드시** 기동합니다. 감사에는 `spec.md`, `plan.md`, `tasks.md`(존재하는 경우), 요구사항과 완료 기준, 변경 diff, 하네스 검증 로그 및 미검증 범위를 제공합니다. 차단 결함이 확인되면 수정 후 관련 검증을 다시 실행하고, 수정이 감사 지적을 직접 해결했거나 변경 범위가 실질적으로 넓어진 경우에만 재감사를 요청합니다.

그 밖에는 사용자가 명시적으로 감사·검수를 지시했거나 보안·데이터 마이그레이션·운영 변경·대규모 설계처럼 실패 비용이 큰 작업일 때만 `auditor`를 선택적으로 기동합니다. `speckit-implement`가 변경 없이 종료된 경우에는 감사 대신 그 이유와 미검증 범위를 기록합니다.
