...
> **🚨 충돌 해결 수칙 (Fail-safe)**: 지침 간 내용이 충돌하거나 해석이 불명확한 경우 독단적으로 진행하지 말고 **[즉시 변경 작업을 멈춘 후]** 충돌 내용과 선택지를 사용자에게 알리고 지시를 기다리십시오.
...
- **기계적 변환 도구 수용**: Linter, formatter, 코드 생성기 등 구조적 변환 도구가 요구하는 기계적 변경은 무단 변경으로 보지 않되, 변경 범위와 예상 영향을 사용자에게 사전에 알린 뒤 수행하십시오.
...
4. **조건부 자가 치유 루프 (Conditional Self-healing)**
   ...
   - **논리적 에러** (테스트 실패/런타임 에러): 즉시 재수정하지 말고, 원인 가설을 설정한 뒤 로그·재현 조건·정밀 테스트로 검증하십시오. 검증 결과와 해결책·영향을 사용자에게 보고한 뒤 승인 대기.
